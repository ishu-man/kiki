...
### WORK ON THIS FIRST THING IN THE MORNING!! TEST IT WITH A NEW ELEVENLABS API KEY TO GET A FEEL FOR WHAT'S COMING!!
def get_user_data(endpoint: str):
  user_data = requests.get(endpoint)
  user_JSON_dict = (user_data.json())[0]
  return user_JSON_dict

topic = get_user_data(user_data_endpoint)["topic"]
context = get_user_data(user_data_endpoint)["context"]
goal = get_user_data(user_data_endpoint)["goal"]


def get_AI_script(topic: str, context: str, goal: str):
  prompt_for_script = f"""
  ...
  response = client.models.generate_content(
    model= 'gemini-flash-latest', # was 2.5 pro earlier - service error!
